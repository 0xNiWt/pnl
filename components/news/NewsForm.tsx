...
export default function NewsForm({ mode, newsId, initialData }: NewsFormProps) {
    ...
    return (
        <form onSubmit={handleSubmit} className="flex flex-col gap-5">
            {error && (
                <div className="p-3.5 rounded-none bg-accent/10 border border-accent/30 flex items-center gap-2.5 text-xs font-medium text-accent">
            ...
            <div>
                ...
                <div className="relative">
                    ...
                    <textarea
                        ...
                        rows={2}
                        className="w-full rounded-none border border-primary/10 bg-primary/5 pl-10 pr-4 py-2.5 text-sm text-primary placeholder:text-primary/30 outline-none transition-all focus:border-accent focus:bg-white focus:ring-2 focus:ring-accent/20 resize-none"
                    />
                </div>
            </div>
            ...
            <div>
                ...
                <textarea
                    ...
                    rows={10}
                    className="w-full rounded-none border border-primary/10 bg-primary/5 px-4 py-3 text-sm text-primary placeholder:text-primary/30 outline-none transition-all focus:border-accent focus:bg-white focus:ring-2 focus:ring-accent/20 resize-y"
                />
            </div>
            ...
            <div>
                <div className="flex items-center justify-between mb-1.5">
                    ...
                    <button
                        ...
                        className="flex items-center gap-1 text-xs font-semibold text-accent hover:text-primary transition-colors"
                    >
                        <Plus size={14} />
                        Додати фото
                    </button>
                        ...
                        <div key={index} className="relative flex items-center gap-2">
                            ...
                            {images.length > 1 && (
                                <button
                                    ...
                                    className="w-9 h-9 flex-shrink-0 rounded-none bg-accent/5 hover:bg-accent/10 flex items-center justify-center text-accent transition-colors"
                                >
                                    <X size={16} />
                                </button>
                            )}
            ...
            <button
                type="submit"
                disabled={loading}
                className="w-full mt-2 rounded-none bg-primary py-3 px-4 text-sm font-bold text-background tracking-wide hover:bg-primary/90 active:scale-[0.99] transition-all flex items-center justify-center gap-2 disabled:opacity-60 disabled:cursor-not-allowed"
            >
